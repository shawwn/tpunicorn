...
class Config(InvokeConfig):
    """
    An `invoke.config.Config` subclass with extra TPUDiePie-related behavior.
    ...
    - it triggers loading of TPUDiePie-specific env vars (e.g.
      ``TPUDIEPIE_RUN_HIDE=true`` instead of ``INVOKE_RUN_HIDE=true``) and
      filenames (e.g. ``/etc/tpudiepie.yaml`` instead of ``/etc/invoke.yaml``).

    .. versionadded:: 0.0.1
    """

    prefix = "tpudiepie"
    env_prefix = "TPUDIEPIE"

    def __init__(self, *args, **kwargs):
        """
        Creates a new TPUDiePie-specific config object.
    ...
    def global_defaults():
        ...
        TPUDiePie only extends this method in order to make minor adjustments and
        ...
        For TPUDiePie-specific modifications and additions to the Invoke-level
        ...
        # TODO: as to that, this is a core problem, TPUDiePie wants split
