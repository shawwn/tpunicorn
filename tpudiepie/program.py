...
from . import __version__ as tpudiepie
from .config import Config
# from .executor import TPUDiePieExecutor


class TPUDiePie(Program):
    def print_version(self):
        super(TPUDiePie, self).print_version()
        # print("Paramiko {}".format(paramiko))
        print("Invoke {}".format(invoke))

    def core_args(self):
        core_args = super(TPUDiePie, self).core_args()
        my_args = [
            # Argument(
            #     names=("S", "ssh-config"),
    ...
    def load_collection(self):
        ...
        if self._remainder_only:
            ...
            super(TPUDiePie, self).load_collection()
    ...
    def no_tasks_given(self):
        ...
        if not self._remainder_only:
            super(TPUDiePie, self).no_tasks_given()
    ...
    def update_config(self):
        ...
        super(TPUDiePie, self).update_config(merge=False)
...
def make_program():
    return TPUDiePie(
        name="TPUDiePie",
        binary="tpu[diepie]",
        binary_names=["tpudiepie", "tpu"],
        version=tpudiepie,
        # executor_class=TPUDiePieExecutor,
        config_class=Config,
    )
